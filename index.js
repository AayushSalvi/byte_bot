...
async function loadMobilenet() { // LOADS THE MOBILENET MODEL FOR TRANSFER LEARNING
  const e = document.getElementById('training_name')
  let model
  ...
  switch (e.options[e.selectedIndex].value) {
    case "MobileNetV2":
      ...
      var loadmodel = await tf.loadLayersModel(model);
      var layer = loadmodel.getLayer('conv_pw_13_relu');
      
      break
    case "NASNetMobile":
      model = '../new-byte-main/models/nasnet/model.json'
      //activation_187
      var loadmodel = await tf.loadLayersModel(model)
      var layer = loadmodel.getLayer('activation_187')
      break
    case "EfficientNetB0":
      model = '../new-byte-main/models/effnet/model.json'
      var loadmodel = await tf.loadLayersModel(model)
      var layer = loadmodel.getLayer('block4a_activation')
      break
    case "DenseNet121":
      model = '../new-byte-main/models/densenet/model.json'
      var loadmodel = await tf.loadLayersModel(model)
      var layer = loadmodel.getLayer('relu')
      break;
  ...
  return tf.model({inputs: loadmodel.inputs, outputs: layer.output});
};
...
async function train() { // TRAINS A MODEL USING WEBCAM COLLECTED IMAGES AND CREATES VISOR DATA
  ...
  model = tf.sequential({
    layers: [
      tf.layers.flatten({inputShape: mobmodel.outputs[0].shape.slice(1)}),
...
async function add_data(controllerDataset, label) { // ADS SAVED WEBCAM IMAGE TO A GIVEN DATASET
  ...
  controllerDataset.addExample(mobmodel.predict(img), label);
  img.dispose();
};
...
async function getPredictedClass() {  // SAVES WEBCAM IMAGE
  ...
  const processedImg = tf.tidy(() => img.expandDims(0).toFloat().div(127).sub(1));
  const activation = mobmodel.predict(processedImg);
...
async function init(){// CALLS FOR WEBCAM INSTANCE AND LOADS MOBILENET
  ...
  mobmodel = await loadMobilenet();
  
  const screenShot = await webcam.capture(); 
  mobmodel.predict(screenShot.expandDims(0)); 
  screenShot.dispose(); 
};
